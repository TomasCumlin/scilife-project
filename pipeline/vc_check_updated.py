...
pearsons=pearsonr(observed_axis, expected_axis)

plt.scatter(observed_axis,expected_axis, color="r", alpha=0.5, edgecolors="red")
plt.plot(observed_axis, reg, '-',alpha=0.8,color="darkblue")
plt.xlabel(f'Observed Variants Frequency (%) of Batch "{batch}"')
plt.ylabel("Expected Variants Frequency (%)")
plt.title("Expected vs Observed Variant Frequency", fontsize=15)
