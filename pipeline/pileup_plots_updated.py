...
def plotting_subst_neighb(bases,batch_name,freq=False):
    ...
    for x in range(int(len(bases)/2)):
        ...
        if freq is True:
            plt.ylabel("Substitution Rate (%)",fontsize=20)
            plt.title(f'Substitution Rates for {subst_string[x]} and {subst_string[-(x+1)]}', fontsize=25)
...
def plotting_subst(subst_data,batch_name):
    ...
    plt.xlabel("Substitution Type", fontsize=10)
    plt.ylabel("# Substitutions",fontsize=10)
    plt.title(f'Substitution Counts for Batch "{batch_name}"', fontsize=13)
    ...
    plt.xlabel("Substitution Type", fontsize=10)
    plt.ylabel("Substitution Rate (%)",fontsize=10)
    plt.title(f'Substitution Frequencies of Batch "{batch_name}"', fontsize=15)
    ...
    plt.xlabel("Substitution Type", fontsize=10)
    plt.ylabel("# Substitutions",fontsize=10)
    plt.title(f'Substitution Counts for Batch "{batch_name}"', fontsize=15)
...
def top_freq(dict_list, batch_name):
    ...
    plt.xlabel("Substitution Type", fontsize=10)
    plt.ylabel("Substitution Rate (%)",fontsize=10)
    plt.title(f'Highest Substitution Rates in Batch "{batch_name}"', fontsize=15)
...
def plotting_comparative(filtered, filteredout, batch_name):
    ...
    plt.xlabel("Substitution Type",fontsize=11,labelpad=10)
    plt.ylabel("# Substitutions",fontsize=10)
...
cmd = f'samtools mpileup -C50 -f {sys.argv[4]} -l {sys.argv[5]} {sys.argv[2]} -Q 0'

pileup_file = os.popen(cmd).readlines()

file_list=[i.split() for i in pileup_file]

cmd2 = f'samtools mpileup -C50 -f {sys.argv[4]} -l {sys.argv[5]} {sys.argv[3]} -Q 0'
